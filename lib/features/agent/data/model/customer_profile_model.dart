...
class CustomerProfileModel extends Equatable {
  ...
  /// Parses the API response: Data is a JSON-encoded string of a list.
  static CustomerProfileModel? fromResponse(dynamic data) {
    ...
    if (parsed is List && parsed.isNotEmpty && parsed.first is Map<String, dynamic>) {
      return CustomerProfileModel.fromJson(parsed.first as Map<String, dynamic>);
    }
    if (parsed is Map<String, dynamic>) {
      return CustomerProfileModel.fromJson(parsed);
    }
    return null;
  }

  String get displayName => customerName ?? customerNameEng ?? '';

  String get formattedBalance => (balance ?? 0).toStringAsFixed(2);

  String get formattedPhone {
    final phones = [tel1, tel2, tel3].where((t) => t != null && t.trim().isNotEmpty).toList();
